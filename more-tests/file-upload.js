...
export default function process({ functionInfo, trigger, event }) {
    try {
        ...
        // {"mimeType":"application/octet-stream","parameters":{},"data":[0,0,1,0,8,8,80,0,123,34,104,34,58,34,52,54,10
        /*
        ...
        */
        ...
        const marker = decoded.data[0] | (decoded.data[1] << 8);

        const firstHeader = {};
        let chunkHeaderOffset = 0;

        if (marker == 0) {
            firstHeader.firstHeaderMarker = marker;
            firstHeader.version = decoded.data[2] | (decoded.data[3] << 8);
            firstHeader.firstHeaderSize = decoded.data[4];
            firstHeader.chunkHeaderSize = decoded.data[5];
            firstHeader.jsonSize = decoded.data[6] | (decoded.data[7] << 8);
            chunkHeaderOffset += 8;

            let jsonStr = '';
            for(let ii = 0; ii < firstHeader.jsonSize; ii++) {
                jsonStr += String.fromCharCode(decoded.data[chunkHeaderOffset + ii]);
            }
            firstHeader.json = JSON.parse(jsonStr);

            chunkHeaderOffset += firstHeader.jsonSize;
        }
        ...
        if (!tempLedgerData.data.files[chunkHeader.fileId.toString()]) {
            ...
            if (firstHeader.version) {
                tempLedgerFile.firstHeader = firstHeader;
                tempLedgerFile.event = {
        ...
        if (contiguousChunks && tempLedgerFile.firstHeader) {
            const lastChunk = tempLedgerFile.chunks[tempLedgerFile.chunks.length - 1];
            if ((lastChunk.chunkOffset + lastChunk.chunkSize) >= tempLedgerFile.firstHeader.json.s) {
                allChunks = true;
            }
        }
        ...
        if (allChunks) {
            ...
            if (hashHex == tempLedgerFile.firstHeader.json.h) {
